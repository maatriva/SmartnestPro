...
const Navbar = () => {
  ...
  return (
    <nav ref={navRef} className="demo-navbar fixed top-0 left-0 right-0 z-50 px-6 py-2 
        ...
        <ul className="flex gap-8 font-semibold">
          {["Home", "Survey", "About Us", "Diseases"].map((text, i) => (
            <li key={i} className="nav-item">
              <Link to="/" className="nav-link inline-block hover:text-[var(--text-dark)] transition duration-200">
                {text}
              </Link>
            </li>
          ))}
